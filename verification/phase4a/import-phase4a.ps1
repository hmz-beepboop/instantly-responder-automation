...
$ResultsPath = Join-Path $RepoRoot 'verification/phase4a/offline-test-results.json'
$SenderPath = Join-Path $RepoRoot 'workflows/03_reply_sender_validation.json'
$ErrorPath = Join-Path $RepoRoot 'workflows/04_reply_error_handler_validation.json'
...
function Assert-OfflineEvidence {
    ...
    if ($Results.total -lt 42 -or $Results.failed -ne 0 -or $Results.passed -ne $Results.total) {
        throw "Offline evidence is not acceptable. Expected at least 42 passing tests and zero failures; found passed=$($Results.passed), total=$($Results.total), failed=$($Results.failed)."
    }
...
function Assert-NoExistingExactNames {
    ...
    if ($Matches.Count -gt 0) {
        ...
        throw "Refusing to create duplicate Phase 4A workflows. Existing exact-name match(es): $Summary"
    }
...
try {
    ...
    $SenderWorkflow = Read-SafeWorkflow -Path $SenderPath
    $ErrorWorkflow = Read-SafeWorkflow -Path $ErrorPath

    if ($SenderWorkflow.name -ne $ExpectedNames[0] -or $ErrorWorkflow.name -ne $ExpectedNames[1]) {
    ...
    $Created += New-InactiveWorkflow -Workflow $SenderWorkflow -SourcePath $SenderPath
    $Created += New-InactiveWorkflow -Workflow $ErrorWorkflow -SourcePath $ErrorPath

    Write-Host ''
    Write-Host 'PHASE_4A_IMPORT_COMPLETE'
    ...
    Write-Host ''
    Write-Host 'PHASE_4A_IMPORT_FAILED'
