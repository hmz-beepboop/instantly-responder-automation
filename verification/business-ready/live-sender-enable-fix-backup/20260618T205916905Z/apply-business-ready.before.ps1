...
$N8nUrl = "https://n8n.hmzaiautomation.com"
...
# Runtime sender constants injected into Workflow 03 code nodes. The
# DRY_RUN flag is intentionally true in the stored safe config and becomes
# false only during the supervised controlled-live acceptance temporary config.
$SenderRuntimeConfig = [ordered]@{
    OPERATING_MODE        = [string]$Config.operating_mode
    DRY_RUN               = [bool]$Config.dry_run
    LIVE_CAMPAIGNS        = @($Config.live_campaigns)
    LIVE_CREDENTIAL_READY = [bool]$Config.live_credential_readiness.instantly
}
...
$InjectionsBySpecKey = @{
    REPLY_SENDER = @(
        [pscustomobject]@{
            NodeName = "A. Validate Decision Engine Output"
            MarkerName = "SENDER_CONFIG"
            VarName = "SENDER_CONFIG"
            Value = $SenderRuntimeConfig
        },
        [pscustomobject]@{
            NodeName = "B. Re-run Send & Suppression Gates"
            MarkerName = "SENDER_CONFIG"
            VarName = "SENDER_CONFIG"
            Value = $SenderRuntimeConfig
        },
        [pscustomobject]@{
            NodeName = "C2. Gate Rejection Terminal"
            MarkerName = "SENDER_CONFIG"
            VarName = "SENDER_CONFIG"
            Value = $SenderRuntimeConfig
        },
        [pscustomobject]@{
            NodeName = "D. Compute Stable Send Key"
            MarkerName = "SENDER_CONFIG"
            VarName = "SENDER_CONFIG"
            Value = $SenderRuntimeConfig
        },
        [pscustomobject]@{
            NodeName = "E2. Normalize Acquisition Result"
            MarkerName = "SENDER_CONFIG"
            VarName = "SENDER_CONFIG"
            Value = $SenderRuntimeConfig
        },
        [pscustomobject]@{
            NodeName = "F2. Blocked Duplicate or Rerun Terminal"
            MarkerName = "SENDER_CONFIG"
            VarName = "SENDER_CONFIG"
            Value = $SenderRuntimeConfig
        },
        [pscustomobject]@{
            NodeName = "G. Suppression Actions (Mock Adapter)"
            MarkerName = "SENDER_CONFIG"
            VarName = "SENDER_CONFIG"
            Value = $SenderRuntimeConfig
        },
        [pscustomobject]@{
            NodeName = "H. Verify Suppression Results"
            MarkerName = "SENDER_CONFIG"
            VarName = "SENDER_CONFIG"
            Value = $SenderRuntimeConfig
        },
        [pscustomobject]@{
            NodeName = "I2. Suppression Escalation Terminal"
            MarkerName = "SENDER_CONFIG"
            VarName = "SENDER_CONFIG"
            Value = $SenderRuntimeConfig
        },
        [pscustomobject]@{
            NodeName = "K2. Build DRY_RUN Terminal Result"
            MarkerName = "SENDER_CONFIG"
            VarName = "SENDER_CONFIG"
            Value = $SenderRuntimeConfig
        },
        [pscustomobject]@{
            NodeName = "N. Live Adapter Contract (Validation-Only, Unreachable)"
            MarkerName = "SENDER_CONFIG"
            VarName = "SENDER_CONFIG"
            Value = $SenderRuntimeConfig
        },
