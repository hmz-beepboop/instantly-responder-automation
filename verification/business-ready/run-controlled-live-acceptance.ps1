...
function ConvertTo-HmzDateTimeOffset {
    ...
    throw "Could not parse n8n execution timestamp '$Value'."
}
...
function Wait-ForControlledReplyOutcome {
    ...
    while ((Get-Date) -lt $Deadline -and $null -eq $Terminal) {
        ...
        foreach ($Exec in $Candidates) {
            ...
            if (ConvertTo-HmzDateTimeOffset -Value $StartedAt -lt $RequestStartUtc) { continue }
