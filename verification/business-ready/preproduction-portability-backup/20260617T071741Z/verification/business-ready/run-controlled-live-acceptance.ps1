...
function Invoke-Preflight {
    ...
    try {
        ...
        $MatchWorkspace = [string](Get-OptionalPropertyValue -Object @($Match)[0] -Name "workspace_id")
...
function Set-RuntimeWorkflowsActive {
    ...
    foreach ($Name in $RuntimeWorkflowNames) {
        $Id = Get-WorkflowIdByName -Summaries $Summaries -Name $Name
        Invoke-RestMethod -Method POST -Uri "$N8nUrl/api/v1/workflows/$Id/$Action" -Headers $Headers | Out-Null
    }
    ...
    $HarnessSummary = @($Summaries | Where-Object { [string]$_.id -eq $HarnessId })[0]
    if ([bool]$HarnessSummary.active) {
        Invoke-RestMethod -Method POST -Uri "$N8nUrl/api/v1/workflows/$HarnessId/deactivate" -Headers $Headers | Out-Null
    }
}
...
function Restore-SafeState {
    ...
    try {
        if (-not [string]::IsNullOrWhiteSpace($env:HMZ_N8N_API_KEY) -and -not [string]::IsNullOrWhiteSpace($env:HMZ_REVIEW_CASES_DATA_TABLE_ID)) {
            Invoke-ApplyBusinessReady -AdditionalEnv @{}
        }
        else {
            $Warnings += "Could not rerun apply-business-ready.ps1 during restore: HMZ_N8N_API_KEY and/or HMZ_REVIEW_CASES_DATA_TABLE_ID not set."
        }
    }
    catch {
