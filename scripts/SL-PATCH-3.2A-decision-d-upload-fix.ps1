...
<#
.SYNOPSIS
    SL-PATCH-3.2A — AbortController fix for Decision node D (upload-fix revision).

.DESCRIPTION
    Same JavaScript patch as SL-PATCH-3.2 (Promise.race replaces AbortController).
    Fixes the HTTP 400 that occurred in 3.2 Apply: 3.2 sent the raw GET response
    object as the PUT body, which includes read-only fields (id, createdAt,
    updatedAt, versionId, active, tags) that n8n's API rejects.
    3.2A builds a filtered PUT body — name/nodes/connections/settings only,
    plus staticData/pinData if present — matching the proven pattern from SL-PATCH-3.1.
...
.PARAMETER WhatIf
    Validate in memory. No API writes.

.PARAMETER Apply
    Apply to production. Run only after WhatIf passes and user types APPLY_PATCH_3_2A.
#>
param(
    [switch]$WhatIf,
    [switch]$Apply
...
function Invoke-Prod {
    ...
    try {
        ...
        # Capture response body for diagnosis (fixes missing detail from SL-PATCH-3.2)
        $respBody = ''
...
# ── STEP 1: AUTH ──────────────────────────────────────────────────────────────
Write-Host "`n[$MODE] SL-PATCH-3.2A starting"
...
if ($WhatIf) {
    ...
    Write-Host "UPLOAD_FIX: PUT body will be filtered (name/nodes/connections/settings only)."
    exit 0
}
...
# FIX vs SL-PATCH-3.2: build filtered PUT body — only fields n8n accepts for updates.
# Sending the raw GET response (as 3.2 did) includes read-only fields (id, createdAt,
# updatedAt, versionId, active, tags) which cause HTTP 400. Pattern proven in SL-PATCH-3.1.
$putBody = [ordered]@{
...
if ($null -ne $decisionWfFull.staticData) { $putBody['staticData'] = $decisionWfFull.staticData }
if ($null -ne $decisionWfFull.pinData)    { $putBody['pinData']    = $decisionWfFull.pinData }

Write-Host "Uploading patched Decision workflow (filtered PUT body)..."
$putR = Invoke-Prod 'PUT' "/workflows/$WF_DECISION" $putBody
