...
<#
.SYNOPSIS
    SL-PATCH-3.3A — Fix AI HTTP call (this.helpers.httpRequest), senderName resolution, HA notification format.

.DESCRIPTION
    Corrected variant of SL-PATCH-3.3. Manual production Code-node test proved:
      typeof this.helpers.httpRequest === "function"   → VERIFIED
      typeof $helpers.httpRequest     === "undefined"  → VERIFIED
      typeof fetch                    === "undefined"  → VERIFIED

    Three proven fixes:
    A. Decision D callAI: replace global fetch with this.helpers.httpRequest
       (typeof-guarded; if unavailable, routes to ai_failed_fallback with clear error)
...
.PARAMETER Apply
    Apply to production. Run only after WhatIf and user signals APPLY_PATCH_3_3A.
#>
param(
    [switch]$WhatIf,
    [switch]$Apply
...
# Manual proof recorded from live Code-node test on production
$manualHelperProven = $true
...
# ── STEP 1: AUTH ───────────────────────────────────────────────────────────────
Write-Host "`n[$MODE] SL-PATCH-3.3A starting"
...
foreach ($wfMeta in $allMeta) {
    ...
    # HTTP helper scan — look for this.helpers.httpRequest
    $full.nodes | Where-Object { $_.type -match 'code|function' } | ForEach-Object {
        ...
        $c  = $_.parameters.$cp
        if ($c -and ($c -match 'this\.helpers\.httpRequest')) {
            $httpHelperFound = $true
            $httpHelperHits += "$($wfMeta.id)/$($_.name)"
            Write-Host "  THIS_HELPERS_HTTPREQUEST_FOUND wf=$($wfMeta.id) node='$($_.name)'"
        }
    }
}
...
# ── STEP 5: MANUAL HELPER PROOF STATUS ─────────────────────────────────────────
Write-Host "`n=== HTTP HELPER STATUS ==="
Write-Host "MANUAL_HELPER_PROOF_RECORDED=YES"
Write-Host "  Proven by live Code node test: typeof this.helpers.httpRequest === 'function'"
Write-Host "  typeof fetch === 'undefined' | typeof `$helpers.httpRequest === 'undefined'"
if ($httpHelperFound) {
    Write-Host "LIVE_NODE_SCAN_FOUND=YES hits=$($httpHelperHits -join ',')"
} else {
    Write-Host "LIVE_NODE_SCAN_FOUND=NO (no existing nodes use it yet — manual proof sufficient)"
}
...
# 8A: replace global fetch with this.helpers.httpRequest (typeof-guarded)
$old_8A = (@'
...
Write-Host "  8A APPLIED (fetch → this.helpers.httpRequest with typeof guard) delta=$(($new_8A.Length - $old_8A.Length))"
...
Check-Item 'dec_typeof_guard_present'               ($patchedDec -match "typeof this\.helpers\.httpRequest !== 'function'")
...
# Manual proof (informational — not blocking since manually proven)
Check-Item 'manual_helper_proof_recorded'           $manualHelperProven

Write-Host ""
Write-Host "STRUCTURAL_CHECKS: $(if ($script:structuralPass){'PASS'}else{'FAIL'}) ($($script:passCount)/$($script:totalCount))"
Write-Host "MANUAL_HELPER_PROOF_RECORDED: $(if ($manualHelperProven){'YES'}else{'NO'})"

$safeToApply = $script:structuralPass -and $manualHelperProven
...
if ($WhatIf) {
    if ($safeToApply) {
        ...
        Write-Host "To apply, type exactly: APPLY_PATCH_3_3A"
    } else {
        Write-Host "`nWHATIF_RESULT: STRUCTURAL_OK_PROOF_MISSING"
        Write-Host "SAFE_TO_APPLY: NO"
    }
    Write-Host "API_CALLS_USED=$($script:Calls)/$WHATIF_LIMIT"
    Write-Host "BACKUP_DIR=$backupDir"
...
$postDecChecks = [ordered]@{
    ...
    'live_dec_this_helpers_httpRequest'     = ($vDecCode -match 'this\.helpers\.httpRequest\(')
    'live_dec_dollar_helpers_absent'        = ($vDecCode -notmatch '\$helpers\.httpRequest')
    'live_dec_typeof_guard'                 = ($vDecCode -match "typeof this\.helpers\.httpRequest !== 'function'")
