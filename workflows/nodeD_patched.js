...
function _5qApplyActiveFormRuleInstructionToDraft(text, microIntent, behaviouralGuidance, senderName, bookingLink) {
  ...
  const instructionUrl = _5qExtractFirstUrl(instruction);

  // EMAIL-CONTENT RULE: instruction contains a URL — treat as direct email content (old style, e.g. c9860e74).
  if (instructionUrl) {
    const parts = [];
    parts.push('Booking link: ' + instructionUrl);
  ...
  // BEHAVIOURAL-CONSTRAINT RULE: no URL in instruction — treat as a policy/constraint spec (e.g. 97eb3b0a).
  // Apply constraints to the existing template text. Do NOT paste instruction sentences as email lines.
  const isConstraintSpec = /replace the previous|do not ask|do not say|do not use/i.test(instruction);
  if (!isConstraintSpec) return text;

  const _bkLink = bookingLink || 'https://calendar.app.google/bNXWJkS3xz3yqdW36';
  let lines = String(text).split(/\n{2,}/).map(function(l){ return l.trim(); }).filter(Boolean);

  // Strip trailing sender name — re-added at end
  let trailSender = '';
  if (senderName && lines.length > 0 && lines[lines.length - 1] === senderName) {
    trailSender = lines.pop();
  }

  // Constraint: remove availability-requesting phrases
  const BAD_AVAIL = [
    /reply with (?:a couple of )?times? that work/i,
    /share (?:your )?availability/i,
    /let me know (?:your )?availability/i,
    /when (?:are you )?available/i,
  ];
  lines = lines.filter(function(l){ return !BAD_AVAIL.some(function(rx){ return rx.test(l); }); });

  // Constraint: strip "if (they|you) have any" endings
  lines = lines.map(function(l){
    return l.replace(/\s*[,.]?\s*(?:if (?:they|you) have any)\.?\s*$/i, '.').replace(/\.{2,}/g, '.');
  });

  // Constraint: strip "I can book you/them in" phrasing
  lines = lines.map(function(l){
    return l.replace(/\bI(?:'ll)? (?:can )?book (?:you|them) in\b/gi, 'you can book a time');
  });

  // Constraint: ensure booking link present once
  const hasLink = lines.some(function(l){ return l.includes(_bkLink) || /https?:\/\//i.test(l); });
  if (!hasLink) {
    lines.push('You can choose a suitable time here: ' + _bkLink);
  }

  // Constraint: invite questions before booking if instruction requests it
  const wantsQuestion = /invit(?:e|ing) questions?|feel free.*question|question.*before/i.test(instruction);
  const hasQuestion = lines.some(function(l){ return /\bquestion/i.test(l); });
  if (wantsQuestion && !hasQuestion) {
    lines.push('Feel free to ask any questions beforehand.');
  }

  // Constraint: no more than 3 content lines before sender name
  while (lines.length > 3) {
    if (lines.length > 1) lines.splice(1, 1);
    else break;
  }

  lines.push(trailSender || senderName || '');
  return lines.filter(Boolean).join('\n\n');
}

function _5qApplyPricingConstraints(text, behaviouralGuidance) {
  if (!text || !behaviouralGuidance) return text;
  if (!/humanapproval_form_created_learning|humanapproval_form/i.test(behaviouralGuidance)) return text;
  if (!/(?:answer.*direct|do not dodge|pricing.*direct)/i.test(behaviouralGuidance)) return text;
  const evasivePricing = /Pricing depends on scope\. I want to give you a number that actually reflects your situation rather than a generic figure\. The best way to do that is a brief 10-minute conversation\./;
  if (!evasivePricing.test(text)) return text;
  const hasPilot   = /small pilot/i.test(behaviouralGuidance);
  const hasPerCall = /pay.per.(?:shown.)?call/i.test(behaviouralGuidance);
  const hasSetup   = /setup fee/i.test(behaviouralGuidance);
  let pricingLine = '';
  if (hasSetup && hasPerCall) {
    pricingLine = 'On pricing: there is an initial setup fee and the ongoing model is per-shown-call — no flat monthly retainer.';
  } else if (hasPerCall) {
    pricingLine = 'On pricing: the ongoing model is per-shown-call rather than a flat monthly retainer.';
  } else if (hasSetup) {
    pricingLine = 'On pricing: the structure includes an upfront setup component.';
  } else {
    pricingLine = 'Happy to give you the specifics on pricing.';
  }
  if (hasPilot) { pricingLine += ' A small pilot is also an option before any larger commitment.'; }
  pricingLine += ' A brief 10-minute conversation is the fastest way to give you accurate figures for your setup.';
  return text.replace(evasivePricing, pricingLine);
}
...
function _5qApplyActiveRuleDraftPostprocessing(text, microIntent, behaviouralGuidance, senderName, bookingLink) {
  ...
  out = _5qApplyPricingConstraints(out, behaviouralGuidance);
  return out;
}
...
for (const item of items) {
  ...
  const templateMicroIntent = microIntent === 'BOOKING_REQUEST' ? 'MEETING_TIME_REQUEST' : (microIntent === 'NON_PRIORITY' ? 'NOT_NOW' : microIntent);
