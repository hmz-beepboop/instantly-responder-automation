...
**Created:** 2026-07-04  
**Updated:** 2026-07-07 Fable Run 3 — **the owner-reported "not-now failures" were themselves false positives in reporting, not real failures.** Live rows (cases 4a5596a0/07bd8bb5/659d1e01) prove the backend succeeded (effective NON_PRIORITY via rule 6e50fd54, AI_DRAFT_APPROVAL, ai_attempt.ok=true, drafts present, upgrade eligible); the operator-facing surfaces under-reported it (chat "Micro intent: N/A"; no Original-vs-Effective display; the review form labelled the EFFECTIVE micro intent "Original micro intent" — an untruthful label that itself invited misdiagnosis; no explicit reply-mode/AI-status line). Fixed as `SL-PHASE-5Q-RUN3-UIVIS` (HumanApproval `0054f20b` → `99b4c092`, nodes J + chat D only). Anti-false-positive guarantees preserved and tested (P21): the AI banner and new "AI draft status" line derive from `draft_source_raw` + `ai_attempt.ok` — a fallback or deterministic draft can never display as an AI success; "AI draft generated and passed validation" requires `ai_attempt.ok===true`; multi-rule injection still refuses per-rule attribution; no backend classification/upgrade logic was changed. Harness 463/463 PASS. Sender untouched (read-only audit only), no Instantly POST, Shadow inactive, Gate 2 unapproved.

**Previous:** 2026-07-07 session 16 — S2 closure. Two false-positive classes fixed with live evidence, one truthfulness class fixed: (1) **Classification-learning false positive (case-5afa61d3):** active OFFER_EXPLANATION→PROOF_REQUEST correction rules (`d82e94d7`/`1dba7933`, created from trust case case-bd8e453e) hijacked a genuine setup question that contained no trust/proof signal — the promotion gate set covered booking and NON_PRIORITY but not PROOF_REQUEST. Fixed: `SL-PHASE-5Q-S2-PROOF-GATE` requires `_5qReplyHasProofTrustIntent(replyText)` before any rule may promote to PROOF_REQUEST/PROOF_OR_CASE_STUDY_REQUEST. Genuine trust variants ("How can I trust you?", "I don't know if you are trustworthy.", "Can you show me evidence...") still pass (P20.10–12). (2) **Metadata false positives:** every case row stored `reply_mode=HUMAN_ONLY` (Decision never emitted reply_mode; Node A default), and case-5afa61d3 stored `reply_draft_status=NO_DRAFT_HUMAN_ONLY` next to a real AI draft (status computed before the upgrade guard). Both fixed (SL-PHASE-5Q-S2-REPLY-MODE, SL-PHASE-5Q-S2-STATUS-SYNC); the sync only flips a stored status when the actual draft contradicts it — NOT_APPLICABLE (suppress/OOO) is never rewritten (P20.30). (3) **False-negative reporting:** single-rule AI prompt injection produced an EMPTY impact summary (case-269eed7f) making real learning look absent. Fixed with a truthful injection summary; multi-rule injection still refuses per-rule attribution (P20.36). The new S2 upgrade engine cannot create hidden false positives: upgrades happen only from an explicit safe allowlist, require ≥1 active form-created style rule for the effective classification, never fire for unsubscribe/legal/hostile/suppress/no-reply/pricing classes, and every decision records `ai_upgrade_eligible` / `ai_upgrade_reason` / `ai_upgrade_blocked_reason`. A fallback draft is never labelled AI (reply_mode maps ai_failed_fallback → FIXED_TEMPLATE_APPROVAL, P20.34; `aiDraftUsedGuidance` still requires draft_source=ai_supervised, P20.44). Decision `4474c96a` → `84b941a4`; HumanApproval unchanged `0054f20b`; harness 425/425 PASS. Sender untouched, no Instantly POST, Shadow inactive, Gate 2 unapproved.

**Previous:** 2026-07-06 session 15 — Dense-paragraph false-positive rejection fixed. The remaining `AI_OUTPUT_VALIDATION_FAILED` frequency on proof/trust cases was live-proven to be a STYLE false positive, not a safety rejection: exec 5329 (PROOF_REQUEST) had a fully safe, honest, correctly-negated AI draft rejected solely by `active policy violation: dense paragraph` (~386-char single paragraph vs 360 threshold). Cause chain: globally-scoped style policy 27293ea8 ("short paragraphs") arms the dense validator for ALL AI drafts, while intInstr.PROOF_REQUEST demanded "One concise paragraph" — the prompt invited exactly what the validator rejects. Siblings 5286/5296 passed at shorter/multi-paragraph shape, confirming the boundary. Fixes: (1) PROOF_REQUEST prompt now asks for 2-3 short paragraphs; (2) style-only dense rejections are repaired by a whitespace-only sentence-boundary reflow and the FULL validator re-runs — safety errors always still fall back (harness P19.11-16 prove invented proof/results/guarantees/pricing still fail after reflow); (3) reflow recorded truthfully (`style_reflow_applied`, `raw_draft_text_before_reflow`); (4) HumanApproval fallback banner now names the exact failed check(s) and states when the rejection was formatting-only, not safety. Deployed: Decision `afe08974` → `4474c96a`; HumanApproval `7aac637e` → `0054f20b`. Harness 375/375 PASS (+26 P19). Known residual (deliberately NOT patched — no live occurrence): FORBIDDEN_AI negation window lacks `can't/cannot/won't` and does not exempt post-keyword negation; logged in `docs/INSTANTLY_RESPONDER_FAULT_LEDGER_AND_SCALE_READINESS.md` item 10(c). Sender untouched, no Instantly POST, Shadow inactive, Gate 2 unapproved.
