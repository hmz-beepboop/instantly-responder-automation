...
**Created:** 2026-07-04  
**Session verdict:** PARTIAL — 4 gaps patched; live regression repair applied; GAP-3b + classification patches applied session 4; valid-fallback submit/reopen repair applied session 10; PROOF_REQUEST draft-learning activation bridge fixed session 11; PROOF_REQUEST AI-fallback non-null fix applied session 12; context/token upstream regression fixed session 13; trust/proof variant classifier repair deployed session 14; dense-paragraph style-only fallback fix deployed session 15; S2 upgrade engine + PROOF_REQUEST promotion gate + truthful metadata deployed session 16; **Run 3 (2026-07-07): session-16 behaviours LIVE-VERIFIED via five fresh production rows — S2 loop closed on evidence for the not-now class; remaining S2 item is the live rollback drill (S2.6)**
**Fable Run 3 update (2026-07-07) — live closure evidence:** the three owner-reported "not-now failures" (cases 4a5596a0 / 07bd8bb5 / 659d1e01: "Not now. Maybe later" / "I can't right now." / "I don't have time right now. Maybe later") were re-fetched via REST and are backend SUCCESSES: baseline `AMBIGUOUS/AMBIGUOUS_SHORT_REPLY` → effective `AMBIGUOUS/NON_PRIORITY` (rule `6e50fd54` applied), `ai_upgrade_eligible=true` (`ACTIVE_DRAFT_LEARNING_RULES_PRESENT_FOR_SAFE_CLASS`), `reply_mode=AI_DRAFT_APPROVAL`, `ai_attempt.ok=true`, drafts present, no fallback. case-58e6b3b0 (trust) and case-5e2fbcbe (setup) also PASS with real AI drafts and truthful metadata/impact summaries. The upgrade engine, promotion gate, reply-mode emission, and summary truthfulness from session 16 are all live-proven. The only defect found was operator-facing visibility (chat "Micro intent: N/A"; no Original-vs-Effective display; mislabelled "Original micro intent"; no explicit AI-draft-status/reply-mode line) — fixed as `SL-PHASE-5Q-RUN3-UIVIS`, HumanApproval `0054f20b` → `99b4c092` (nodes J + chat D only; Decision unchanged `84b941a4`). Harness 463/463 PASS (+38 P21: exact live phrases, UI-visibility markers, truthful-label checks, negative controls for booking/pricing/proof/unsubscribe/legal/hostile, never-upgrade protections, node --check, no-Instantly-POST). No classifier/upgrade/style patch was made for the five rows — none was needed.  
**Session 16 update (2026-07-07) — S2 closure:** the three owner-reported live cases were traced from production rows. Verdicts: case-64589b37 (not-now) — learning fully consumed (rule 6e50fd54 classification + 877c3d75/cdada69d post-processor delta with visible draft effect) but the class stayed deterministic because the upgrade guard covered PROOF_REQUEST only; case-269eed7f (trust) — real AI-supervised draft with rule ea15095a injected, only truthfulness gaps (empty impact summary, row reply_mode=HUMAN_ONLY); case-5afa61d3 (setup) — classification hijack: active OFFER_EXPLANATION→PROOF_REQUEST correction rules (d82e94d7/1dba7933) fired on a reply with zero trust/proof signal because `_5qClassificationRuleAllowedForReply` had no PROOF_REQUEST promotion gate; stale `reply_draft_status=NO_DRAFT_HUMAN_ONLY` sat next to a real AI draft. Fixes (Decision Node D only, `4474c96a` → `84b941a4`): PROOF_REQUEST promotion gate; generalized safe deterministic/human→AI upgrade engine (allowlist PROOF_REQUEST/NON_PRIORITY/NOT_NOW; unsubscribe/legal/hostile/suppress/no-reply/pricing classes never upgrade; auditable via `ai_upgrade_eligible`/`ai_upgrade_reason`/`ai_upgrade_blocked_reason`/`effective_classification_used_for_draft_policy`); NON_PRIORITY/NOT_NOW AI prompt instructions; truthful reply_draft_status sync; Decision now emits `decision.reply_mode` (fixes the row-level HUMAN_ONLY default for every case); single-rule AI-injection impact summary now non-empty and truthful, multi-rule stays attribution-uncertain. Rule-id audit lists reuse existing fields (`considered` = `active_learning_rules_found`, `consumed` = `applied_learning_rule_ids`). Harness 425/425 PASS (+50 P20). HumanApproval untouched (`0054f20b`). Sender untouched; no Instantly POST; Shadow inactive; Gate 2 unapproved.

**Rule rollback / deactivation procedure (S2.6, documented + offline-drilled session 16):** every learning rule lives in Q12 (`sl_rule_candidates`, DataTable `CSdiTjXfi0tl0oZF`) with `rule_id`, `source_case_id`, and `status`. Decision consumes only rows with `status` in {active, effective} (`_q12DynamicFormClassificationRules` filter and behavioural-policy fetch). To roll back a bad rule the operator sets its `status` to `deactivated` (or `rejected`/`superseded`) in the n8n Data Table UI or via the REST rows endpoint — the next case ignores it immediately; no workflow deploy is needed. Newer same-scope rules override older ones automatically (`newestByScope`, time-descending). A wrong classification promotion can also be neutralised without touching Q12 because content gates (booking / NON_PRIORITY / PROOF_REQUEST promotion) refuse rules that do not match the reply signal. Offline drill proof: harness P20.38–P20.40 (deactivated rule excluded by status filter; newer rule wins; deactivated rule no longer selected). Live drill (owner flips one rule status and sends a probe email) remains the last S2.6 step.
